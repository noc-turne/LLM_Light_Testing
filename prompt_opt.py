...
def process_topic(background_name, messages, topic, topic_chosen_list, save_path, expand_num, extend_num, user_prompt_generator):
    local_messages = messages[:]
    topic_hist_list = [topic]

    # Generate user prompt
    if user_prompt_generator == UserPromptGenerator.preset:
        user_prompt_text = choose_from_preset(topic)
...
def dfs_generate_tree(background_name, messages, topic_hist_list, depth, topic_chosen_list, save_path, expand_num=2, extend_num=6, user_prompt_generator=UserPromptGenerator.preset):
...
if __name__ == "__main__":
    ...
    background = [
        ...
        {"role": "assistant", "content": "在的，有什么需要帮忙的吗？"}
    ]

    # You can choose the background name based on your requirements
    selected_background = background_name_list[1]  # For example, "求助"

    dfs_generate_tree(
        background_name=selected_background, 
        messages=background, 
        topic_hist_list=[], 
        depth=0, 
        topic_chosen_list=topic_chosen_list, 
        ...
        expand_num=2, 
        extend_num=6, 
        user_prompt_generator=UserPromptGenerator.AI
    )
