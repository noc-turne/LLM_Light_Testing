...
class Proactivity:
    ...
    def get_from_memory(self) -> json:
        ...
        return json.dumps({"关键事实": fact, "超限对话总结": summary, "关键事实要素": elements}, ensure_ascii=False)
...
class LanguageModelAPIHandlerProactivity(LanguageModelAPIHandler):
    ...
    def _before_process(self, prompt: str, count: int) -> List[Dict[str, str]]:
        ...
        for interaction in self.proactivity_chat_helper.get_history_chat():
            if interaction != '':
                for chat in interaction:
